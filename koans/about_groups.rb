...
class AboutGroups < EdgeCase::Koan
  ...
  def test_distinct
    ...
    assert_equal [60606, 60611].sort, @addresses.distinct('zip'), "Return distinct fields different wrong"
  ...
  def test_simple_aggregation
    assert_equal [{"city"=>"chicago", 'zsum' => 3300.0}, {"city"=>"decatur", 'zsum' => 3006.0}], 
  ...
  def test_aggregation_with_finalize
    assert_equal [{"city"=>"chicago", "avg_pop"=>1100.0, "zsum"=>3300.0, "zc"=>3.0}, 
	  ...
	  @zips.group([:city], {}, { 'zsum' => 0, 'zc' => 0, 'avg_pop' => 0 }, 'function(doc,out) { out.zsum += doc.population; out.zc += 1; }', true, 
